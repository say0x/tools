...
- Beliebig viele frei konfigurierbare Kredite nebeneinander (Start: 2, per "+ Kredit hinzufügen"/"Entfernen" erweiter-/reduzierbar, mindestens einer bleibt immer stehen), nutzt direkt `berechneTilgungsplan` (`src/server/calc/financing/tilgungsplan.ts`) — dieselbe Funktion wie im Immobilien-Rechner, hier aber ohne dass ein Objekt existieren muss.
...
- `src/lib/chart-colors.ts` (`farbeFuerIndex`): kuratierte Palette für die ersten 8 Objekte, danach Goldener-Winkel-Verteilung (137,508°) statt eine kurze Palette zyklisch zu wiederholen — verhindert, dass sich ab dem 9. verglichenen Objekt Farben doppeln (Objektvergleich, Kreditvergleich). Im Objektvergleich nutzen die React-Keys der Charts die Objekt-ID statt des Namens, da Namen nicht garantiert eindeutig sind (z. B. nach Duplizieren). Der Kreditvergleich hat kein `id`-Feld — dort ist die Array-Position als Key/`dataKey` trotz Hinzufügen/Entfernen unproblematisch, weil jedes Eingabefeld vollständig kontrolliert ist (Wert kommt bei jedem Render frisch aus dem `kredite`-Array, kein eigener unkontrollierter DOM-Zustand pro Zeile, der beim Index-Sprung verloren gehen könnte).
